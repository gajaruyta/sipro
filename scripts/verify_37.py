...
def audit_imports_and_states():
    ...
    for name, src in files.items():
        ...
        helpers = {h for h in re.findall(r"\b(dayRange|deltaText|durationText|needsDelta|"
...
def audit_single_math():
    ...
    check("shift_days" not in re.sub(r"[\"'`][^\"'`]*shift_days[^\"'`]*[\"'`]", "", fe_all)
          or "preview.shift" not in fe_all,
          "frontend tidak mengarang pergeseran sendiri")

...
## ✅ STATUS (update terakhir — **pemulihan repo ke-5** + **FASE 36 DITUTUP**)
- **Pemulihan repo (sesi ini, ke-5)**: `/app` kembali ditemukan kosong/template → repo GitHub `sipro` dipulihkan lagi.
...
## 🚧 FASE 37 — KALIBRASI SEKALI KLIK (dari Analitik Telat langsung ke template) — SEDANG DIKERJAKAN

### Masalah nyata yang ditutup
- Analitik Telat (Fase 32) sudah memberi **rekomendasi kalibrasi**, tetapi ujungnya hanya teks
  *"Buka Template Jadwal → ubah hari mulai/selesai langkah ini"*. Supervisor harus pindah layar,
  mencari langkahnya, lalu **mengetik ulang seluruh template** (`PUT /build/templates/{id}` menuntut
  payload penuh) → dalam praktiknya kalibrasi **tidak pernah dilakukan** dan template tetap tidak realistis.
- Tidak ada **jejak kalibrasi**: siapa mengubah durasi, kapan, atas dasar data apa, dan alasannya.
  Tidak ada jalan **kembali** bila kalibrasi ternyata salah.
- **Kebutaan model yang baru ketahuan saat merancang fase ini:** `wait_days` (waktu tunggu wajib,
  mis. curing beton 3 hari) **hanya menahan gerbang kesiapan saat berjalan**, tetapi **tidak pernah
  masuk ke tanggal rencana** (`day_from/day_to`). Jadi rencana secara sistematis **optimistis**:
  pekerjaan yang wajib menunggu curing dinyatakan "telat" padahal fisika betonnya yang tidak bisa dipaksa.
  Ini penyebab keempat rekomendasi `wait_time` yang muncul pada data demo.
...
Fase 31, 32, 33, 34, 35, **dan 36 selesai & terverifikasi**. Urutan berikutnya:
1. **Fase 37 — Kalibrasi Sekali Klik**: dari Analitik Telat langsung mengubah durasi / waktu tunggu
   template pekerjaan (dengan pratinjau dampak + alasan + audit; jadwal berjalan tidak ikut berubah
   diam-diam). **SEDANG DIKERJAKAN.**
2. **Kurva-S & laporan portofolio lintas proyek** untuk direksi (laporan mingguan masih per proyek).
3. **Ringkasan laporan mingguan via WhatsApp** (butuh kredensial Meta; WA masih simulasi).
...
- **Setelah pull/restore repo (PENTING — sudah EMPAT kali terjadi):**
  ...
  4. `bash scripts/seed_reset.sh` → harus **OVERALL PASS (17 gates)**.
...
- Sebelum menyatakan sebuah fase selesai, WAJIB hijau semua:
  - `bash scripts/run_all_gates.sh` (**17 gates**, sudah termasuk `verify_36.py`)
  - `poc_31.py` (63) + `poc_32.py` (79) + `poc_33.py` (66) + `poc_34.py` (57) + `poc_35.py` (43) + `poc_36.py` (**132**)
